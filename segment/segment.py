...
def segment3(ali, win_size=500, theta=1.82):
    ...
    while True:
        ...
        if win_start >= win_last or end_frame > len(ali):
            break
        ...
        if p <= 5:
            ...
            if n_len > len(df_ali.columns) - 2:
                ...
                if sil_b != sil_i != sil_e:
                    change = phones.loc[sil_i -
                                        1].index != phones.loc[sil_i + 1].index
                    try:
                        glrs.append(
                            calc_glr(df_ali, (sil_b, sil_i, sil_e), theta))
...
def test_lbl(name, fn=dsd, thr=2.0, win=200, theta=1.83):
    ...
    ali = preprocess(name)
    vcd = ali.loc[ali.vad].reset_index(
        drop=True).drop(['ord', 'phon', 'vad'], axis=1)
    lbl = vcd.loc[vcd.turn.diff() > 0].index
    raw = vcd.drop('turn', axis=1).values

    calc = np.array([(n + win, fn(i[:win], i[win:]))
                     for n, i in enumerate(sa(raw, win * 2, win * 2 - 1, axis=0))])
...
def seg_lbl(wav_path, txt_path, name, fn=kl2, thr=2.0, win=200, theta=1.83):
    ...
    ali = pd.concat((pd.DataFrame(mfc), pd.Series(
        vad, name='vad'), pd.Series(idx, name='uttid')), axis=1)
