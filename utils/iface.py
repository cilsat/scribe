...
def ali2df(ali_file, raw='phon', fold=None):
    if raw == 'phon':
        df = pd.read_csv(ali_file, delimiter=' ', header=None, index_col=0, usecols=[
                         0, 2, 3, 4], names=['index', 'pos', 'dur', 'phon'])
        df.loc[:, ['pos', 'dur']] = (
            df.loc[:, ['pos', 'dur']] * 100).astype(np.uint16)
        ...
        df = pd.DataFrame([fr.strip().split()
                           for r in raw for fr in r[1:]], dtype=np.float16)
        df.index = [n for r in raw for n in [
            r[0].split('[')[0].strip()] * (len(r) - 1)]
        df.columns = ['eng', 'c1', 'c2', 'c3', 'c4', 'c5',
...
def srt2df(srt_file, frame_len=10):
    ...
    srt = pd.DataFrame.from_dict(
        {int(n[0]) - 1: [n[1]] + n[3:] for n in [i.split() for i in raw]}, orient='index')
    srt.columns = ['start', 'end', 'spkr']

    def f(x): return (3600000 * int(x[:2]) + 60000 *
                      int(x[3:5]) + 1000 * int(x[6:8]) + int(x[-3:])) / frame_len
...
def gen_rand(wavpath, txtpath, outpath, write=False, nfiles=5, spontan=True, delta=True):
    ...
    wavs = np.random.choice(wavs.stdout.decode(
        'utf-8').splitlines(), nfiles).tolist()
    ...
    if write:
        ...
        scp = pd.DataFrame(list(zip(lens, txts, wavs)),
                           index=utts, columns=['length', 'text', 'path'])
        return scp, feats
...
def read_lium_gmm(gmm):
    from sklearn.mixture import GaussianMixture
    with open(gmm, 'rb') as g:
        if g.read(8) == b'GMMVECT_':
            return
