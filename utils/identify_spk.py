...
def main():
    ...
    res = all(args.data_path, args.ubm_path, args.exp_path, args.lium_path, args.duration, args.stage)
...
def all(data, ubm, exp, lium, dur, stage):
    ...
    if stage < 1:
        ...
        spk = make_spk(dfs.loc[dfs.lbl > 0], out=src, col='cls', min_dur=dur*100)
    ...
    if stage < 3:
        ...
        args = [(lium, os.path.join(exp, n+'.t.seg'), 
            os.path.join(data, n+'.wav'), os.path.join(exp, n+'.i.seg'), 
...
def test(lium, seg, wav, iseg, gmm, ubm, name, log):
    ...
    with open(log, 'w') as f: run(cmd, stderr=f)
