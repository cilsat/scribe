...
function Dictate(_config) {
  var configDefault = {
    server: "ws://kak.duckdns.org:6600",
  ...
  this.readChunks = function() {
    ...
    var chunk = this.audioFile.slice(
      this.samplesOffset,
  ...
  this.onLoadChunk = function(e) {
    if (this.samplesOffset >= this.audioFile.size) {
      ...
      // we don't need to close websocket connection manually
      // let server close websocket when transcription has completed
      return;
    }
  ...
  this.onMediaStream = function(stream) {
    ...
    if (!AudioContext) console.log("AudioContext unavailable");
    ...
    if (!this.mic) {
      this.mic = this.audioContext.createScriptProcessor(
        this.bufferSize,
        this.inputChannels,
        this.outputChannels
      );
  ...
  this._exportDataBufferTo16Khz = function(bufferNewSamples) {
    ...
    for (
      i = 0;
      i + filter.length - 1 < buffer.length;
      i = Math.round(samplingRateRatio * nOut)
    ) {
      var sample = 0;
