...
model=/home/cilsat/src/kaldi-offline-transcriber/models
ubm=$model/ubm.gmm
...
$java -Xmx4096m -classpath "$LOCALCLASSPATH" fr.lium.spkDiarization.programs.MDecode --trace --help  --fInputDesc=audio2sphinx,1:3:2:0:0:0,13,0:0:0 --fInputMask=$features --sInputMask=$iseg --sOutputMask=$pmsseg --dPenality=10,10,50 --tInputMask=$pmsgmm $show


#GLR based segmentation, make small segments
$java -Xmx4096m -classpath "$LOCALCLASSPATH" fr.lium.spkDiarization.programs.MSeg   --kind=FULL --sMethod=GLR --trace --help --fInputMask=$features --fInputDesc=$fDesc --sInputMask=$datadir/%s.i.seg --sOutputMask=$datadir/%s.s.seg  $show
...
$java -Xmx4096m -classpath "$LOCALCLASSPATH" fr.lium.spkDiarization.tools.SSplitSeg --help  --sFilterMask=$pmsseg --sFilterClusterName=iS,iT,j --sInputMask=$fltseg --sOutputMask=$splseg --fInputMask=$features --fInputDesc=audio16kHz2sphinx,1:3:2:0:0:0,13,0:0:0 --tInputMask=$sgmm $show

#-------------------------------------------------------------------------------
#Set gender and bandwith
gseg=$datadir/$show.g.$h.seg
$java -Xmx4096m -classpath "$LOCALCLASSPATH" fr.lium.spkDiarization.programs.MScore --help  --sGender --sByCluster --fInputDesc=$fDescCLR --fInputMask=$features --sInputMask=$splseg --sOutputMask=$gseg --tInputMask=$ggmm $show
